export const projects = [
  {
    id: 1,
    title: "Fitness Warrior App",
    description: "AI-powered fitness tracking app with 95% accuracy in activity detection",
    tech: ["React Native", "TypeScript", "Firebase", "TensorFlow.js"],
    image: "/fitness-warrior.jpg", // Main image for card display
    images: [
      "/fitness-warrior.jpg",
      "/fitness-warrior-dashboard.jpg",
      "/fitness-warrior-step.jpg",
      "/fitness-warrior-nutrition.jpg",
      "/fitness-warrior-recommendation.jpg",
      "/fitness-warrior-leaderboard.jpg",
      "/fitness-warrior-userprofile.jpg",
      "/fitness-warrior-analytics.jpg"
    ],
    githubUrl: "https://github.com/Yasindu20/fitness-warrior",
    features: [
      "AI-powered step counter with 95% accuracy",
      "Real-time calorie tracking with food database",
      "Personalized fitness goals",
      "Firebase real-time sync"
    ]
  },
  {
    id: 2,
    title: "Movie Explorer",
    description: "MERN stack movie discovery platform with AI-powered recommendations",
    tech: ["React", "Node.js", "MongoDB", "Express", "AI Integration"],
    image: "/movie-explorer.jpg",
    images: [
      "/movie-explorer.jpg",
      "/movie-explorer-details.jpg",
      "/movie-explorer-watch.jpg",
      "/movie-explorer-recommendations.jpg",
      "/movie-explorer-watchlist.jpg",
      "/movie-explorer-blog.jpg",
      "/movie-explorer-aibot.jpg",
      "/movie-explorer-bottom.jpg"
    ],
    githubUrl: "https://github.com/Yasindu20/movie-explorer",
    features: [
      "AI-powered review system",
      "Personalized recommendations",
      "Mood-based discovery",
      "Real-time streaming availability"
    ]
  },
  {
    id: 3,
    title: "Jumpy Math Game",
    description: "Interactive 2D platformer combining math challenges with gameplay",
    tech: ["JavaScript", "HTML5 Canvas", "Game Development"],
    image: "/jumpy-math.jpg",
    images: [
      "/jumpy-math.jpg",
      "/jumpy-math-gameplay.jpg",
      "/jumpy-math-levels.jpg",
      "/jumpy-math-api.jpg",
      "/jumpy-math-pause.jpg",
      "/jumpy-math-leaderboard.jpg",
      "/jumpy-math-credits.jpg"
    ],
    githubUrl: "https://github.com/Yasindu20/jumpy-math-game",
    features: [
      "500+ dynamic math problems",
      "Responsive double jump system",
      "Progressive difficulty levels",
      "25% improved completion rates"
    ]
  }
];

export const skills = {
  frontend: ["React", "React Native", "JavaScript", "TypeScript", "Next.js", "Tailwind CSS", "HTML5", "CSS3"],
  backend: ["Node.js", "Express.js", "PHP", "Java", "RESTful APIs"],
  database: ["Firebase", "MongoDB", "MySQL"],
  tools: ["Git", "GitHub", "VS Code", "Postman", "Android Studio"],
  other: ["Machine Learning", "TensorFlow.js", "Agile", "CI/CD"]
};

export const personalInfo = {
  name: "Yasindu Dasanga De Mel",
  title: "Full Stack Developer",
  subtitle: "Crafting Digital Experiences with Code & Creativity",
  email: "yasindudemel@gmail.com",
  phone: "+94 770-156-827",
  location: "Panadura, Sri Lanka",
  github: "https://github.com/Yasindu20",
  linkedin: "https://www.linkedin.com/in/yasindudemel/"
};